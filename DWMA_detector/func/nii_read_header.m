...
while(test)
    ...
    info.DataType=fread(fid, 10, '*char')';
    info.DbName=fread(fid, 18, '*char')';
    info.Extents=fread(fid,1,'int');
    info.SessionError=fread(fid,1,'uint16');
    info.Regular=fread(fid, 1, '*char')';
    info.DimInfo=fread(fid, 1, '*char')';
...
info.SliceCode=fread(fid, 1, 'uint8')';
...
info.Descrip=fread(fid, 80, '*char')';
info.AuxFile=fread(fid, 24, '*char')';
...
info.IntentName=fread(fid, 16, '*char')';
info.Magic=fread(fid, 4, '*char')';
