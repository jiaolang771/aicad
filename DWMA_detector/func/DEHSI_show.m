...
%%
dpara = inputdlg([{'Intensity cutoff of interest'}, {'Interested slice start from'},{'Interested slice to'}],...
    'Enter parameter for DWMA image visulization',repmat([1 50],1,1),...
    [{'1.4'}, {'67'},{'70'}]);
...
slice_a= str2double(dpara{2})+1;
slice_b= str2double(dpara{3})+1;
...
%%    display
dwma_display_color(Brain,Combo_2D_abnorm,slice_a,slice_b);
...
f = msgbox({'DWMA Quantification Completed';
    ['Volume on slice [' num2str(slice_a-1) '~' num2str(slice_b-1) ']: ' num2str(round(sum_volume,2))];});
